...
ssh -p 18765 -o StrictHostKeyChecking=no -i "$SSH_KEY" $REMOTE_USER "
    ...
    # Add is_deleted column to rewards table if not exists
    ...
    mysql -h localhost -u ughgtdncr7ro5 -p'mrL*1*P7ke&f' db4ccgnbnclgjg -e \"ALTER TABLE redemptions ADD COLUMN ip_address VARCHAR(45) NULL DEFAULT NULL;\" 2>/dev/null || true
