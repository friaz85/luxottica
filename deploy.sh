...
ssh -p 18765 -o StrictHostKeyChecking=no -i "$SSH_KEY" $REMOTE_USER "
    ...
    mysql -h localhost -u ughgtdncr7ro5 -p'mrL*1*P7ke&f' db4ccgnbnclgjg -e \"ALTER TABLE users ADD COLUMN IF NOT EXISTS password_encrypted TEXT NULL AFTER password_hash;\" 2>/dev/null || true
    # Add vigencia_area column to rewards table (position for printing vigencia date on PDF)
    mysql -h localhost -u ughgtdncr7ro5 -p'mrL*1*P7ke&f' db4ccgnbnclgjg -e \"ALTER TABLE rewards ADD COLUMN IF NOT EXISTS vigencia_area VARCHAR(500) NULL DEFAULT NULL;\" 2>/dev/null || true
"
